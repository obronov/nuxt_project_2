<template>
<div>
  <TitleBoxWithImage :meta="this.$store.state.meta" :showLink="true" :info="history"/>
  <CompanyExperience :history="history"/>
...
export default {
  ...
  async fetch() {
    ...
    /* this.history = await this.fetchData(process.env.fakeUrl + `history?lang=${lang}`) */
    this.history = await this.fetchData(process.env.baseUrl + `combined/history?lang=${lang}`)
