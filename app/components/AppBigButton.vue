<template>
  <div>
    <button 
      type="button"
      class="link" 
      :class="{gray: info.colorSchema == 'gray'}"
      @click="$emit('onClick')"
    >
      <span class="link__header">
        <span class="link__title" v-if="info.title" v-html="info.title"></span>
        <span class="link__text" v-if="info.text" v-html="info.text"></span>
      </span>
      <span class="link__footer" :class="{notIcon: !info.icon}">
        <svg v-if="info.icon == 'arrow'" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
.link__footer{
  ...
  & svg{
    ...
    width: 2.4em;
    height: 2.4em;
    margin:  .5em 0 2.2em  4em;
  }
}

.link__header{
  padding: 3.2em 3.2em 0.5em;
  display: block;
}
